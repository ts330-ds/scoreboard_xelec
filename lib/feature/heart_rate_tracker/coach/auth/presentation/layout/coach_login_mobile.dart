...
class CoachLoginMobile extends StatefulWidget {
  const CoachLoginMobile({super.key});

  @override
  State<CoachLoginMobile> createState() => _CoachLoginMobileState();
}

class _CoachLoginMobileState extends State<CoachLoginMobile> {
    ...
    return BlocListener<CoachAuthCubit, CoachAuthState>(
      listener: (context, state) {
        if (state.status == CoachAuthStatus.authenticated) {
          ...
          context.pushReplacement(HeartTrackerPaths.coachRegistration);
  ...
  Widget _buildEmailField() {
    return TextFormField(
      ...
      decoration: _inputDecoration(label: 'Email', hint: 'Enter your email', icon: Icons.email_outlined),
      validator: (v) {
  ...
  Widget _buildRegisterLink(BuildContext context) {
    return Row(
      ...
      children: [
        const Text("Don't have an account? ", style: TextStyle(color: AppColors.subtext, fontSize: 14)),
        GestureDetector(
          onTap: () => context.push(HeartTrackerPaths.coachRegistration),
          child: const Text(
            'Register',
            style: TextStyle(
  ...
  void _submit(BuildContext context) {
    ...
    context.read<CoachAuthCubit>().login(
