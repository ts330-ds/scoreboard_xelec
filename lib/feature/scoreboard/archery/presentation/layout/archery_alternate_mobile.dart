...
class ArcheryAlternateMobile extends StatelessWidget {
    ...
    return MultiBlocProvider(
      ...
      child: PopScope(
        ...
        onPopInvokedWithResult: (didPop, result) async {
          if (didPop) return;
          // System back disabled; handled via AppBar back button only.
        },
        child: Scaffold(
          ...
          appBar: AppBar(
            leading: IconButton(
              ...
              onPressed: () {
                sl<BleService>().send(sl<ArcheryBleMapper>().resetIDLE());
                context.pop();
              },
            ),
            title: const Text(
                    ...
                    MultiBlocListener(
                      listeners: [
                        ...
                        >(
                          ...
                          listener: (context, state) {
                            final timerCubit = context
                                .read<ArcheryAlternateGameTimerCubit>();
                            _playCompletionBuzzer();
                            timerCubit.stop();
                          },
                        ),
                        BlocListener<
                        ...
                        >(
                          listenWhen: (previous, current) =>
                              previous.status !=
                                  AlternateTimerStatus.finished &&
                              current.status == AlternateTimerStatus.finished,
                          listener: (context, timerState) {
                            ...
                            final timerCubit = context
                                .read<ArcheryAlternateGameTimerCubit>();
                            final controllerCubit = context
                                .read<ArcheryAlternateGameControllerCubit>();
                            if (timerState.activeSide ==
                                ArcheryAlternateSide.left) {
                              controllerCubit.registerLeftWin();
                            } else {
                              controllerCubit.registerRightWin();
                            }
                            if (controllerCubit.state.isComplete) {
                              return;
                            }
                            timerCubit.stop();
                            timerCubit.switchSide(nextSide);
                            timerCubit.start();
                            controllerCubit.setActiveSide(nextSide);
                          },
                        ),
                      ],
                      child: Padding(
  ...
  Future<void> _playCompletionBuzzer() async {
    for (var i = 0; i < 3; i++) {
      sl<BleService>().send(sl<ArcheryBleMapper>().triggerBuzzer());
